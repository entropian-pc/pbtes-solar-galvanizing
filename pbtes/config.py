...
class TESConfig:
    ...
    wall_thickness: float = 0.045     # m   — ASME VIII-1 min for SS316 at 5 bar, 600 °C (D=7 m)
    insulation_thickness: float = 1.0  # m — high-performance multi-layer (ceramic fiber + microporous + mineral wool)
    ...
    # ── Tank materials ─────────────────────────────────────────────────
    tank_conductivity: float = 18.0   # W/(m·K) — SS316 (A240-316) at 500 °C
    insulation_conductivity: float = 0.012  # W/(m·K) — high-performance microporous/aerogel system (1.0 m)
    ...
    # ── Initial condition ──────────────────────────────────────────────
    initial_temperature: float = 300.1  # °C — cold start; warm-up pre-run provides
                                     # realistic initial condition. Do NOT use 460°C.
...
class HTFConfig:
    ...
    T_max: float = 700.0              # °C — CoolProp property upper bound for INCOMP::NaK (NOT operating limit; operating limit is 600 °C via T_max_discharge=580 °C)
...
class EconomicsConfig:
    """LCOH and CAPEX/OPEX calculation parameters.

    All costs in 2025 USD. Cost correlations are documented in
    ``insumos paper/ECONOMIC_METHODOLOGY.md``.
    """

    # ── Financial parameters ────────────────────────────────────────────
    discount_rate: float = 0.08       # Discount rate (8%)
    lifetime: int = 25                # Plant economic lifetime [years]
    om_rate_fraction: float = 0.02    # Annual O&M as fraction of total CAPEX

    # ── CEPCI escalation ────────────────────────────────────────────────
    cepci_current: float = 800.0      # CEPCI 2025 (est.)
    cepci_base: float = 500.0         # CEPCI base for Turton correlation (~2006)

    # ── PTC field costs ─────────────────────────────────────────────────
    # Source: Akar & Kurup (2025), NREL DFMA model
    ptc_specific_cost_ref: float = 300.0   # USD/m² at A_ref = 1000 m²
    ptc_cost_ref_area: float = 1000.0      # Reference aperture area [m²]
    ptc_scale_exponent: float = 0.80       # Economy-of-scale exponent n
    ptc_bop_factor: float = 1.40           # Balance-of-plant multiplier

    # ── TES tank costs ──────────────────────────────────────────────────
    # Source: ASME VIII-1, MEPS 2025, Turton (2018)
    tank_material: str = 'SS316'           # Vessel material
    tank_ss316_price_per_kg: float = 5.50  # SS316 plate [USD/kg] (MEPS mid-2025)
    tank_steel_density: float = 8000.0     # Steel density [kg/m³]
    tank_design_pressure_pa: float = 500000.0  # Design pressure [Pa] (5 bar)
    tank_weld_efficiency: float = 0.85     # ASME weld joint efficiency
    tank_corrosion_allowance_m: float = 0.0015  # Corrosion allowance [m]
    tank_allowable_stress_pa: float = 50e6  # SS316 allowable stress at 600°C [Pa]

    tank_vessel_epc_factor: float = 3.5    # Bare material → installed vessel
    tank_insulation_epc_factor: float = 1.5
    tank_fill_epc_factor: float = 2.0

    # Insulation (high-performance multi-layer system, 600°C hot face, 1.0 m total)
    # Thermal model uses k_eff = 0.012 W/(m·K) — idealized effective conductivity
    insulation_cost_ceramic_per_m2: float = 55.0      # Ceramic fibre 200mm hot face [USD/m²]
    insulation_cost_microporous_per_m2: float = 150.0  # Microporous silica/aerogel board 200mm [USD/m²]
    insulation_cost_mineral_per_m2: float = 40.0       # Mineral wool 500mm outer [USD/m²]
    insulation_cost_cladding_per_m2: float = 40.0      # Al sheet cladding [USD/m²]

    # Fill material
    fill_cost_per_tonne: float = 60.0      # Crushed rock delivered [USD/tonne]

    # Civil works fraction of vessel material cost
    civil_works_fraction: float = 0.18     # Foundation + containment

    # Piping, valves, instrumentation fraction of direct equipment
    piping_valves_fraction: float = 0.10   # 10% of direct equipment subtotal

    # ── HTF (Solar Salt) costs ───────────────────────────────────────────
    # Source: CSP literature, mid-scale industrial pricing
    htf_cost_per_kg: float = 1.00          # Solar Salt [USD/kg]
    htf_density_for_mass: float = 1875.0   # Avg density 300-600°C [kg/m³]

    # ── Heat exchanger cost parameters ──────────────────────────────────
    # Source: Turton et al. (2018), Ch.7, Appendix A
    # Floating-head shell-and-tube, SS316/SS316
    hx_k1: float = 4.8306
    hx_k2: float = -0.8509
    hx_k3: float = 0.3187
    hx_b1: float = 1.63
    hx_b2: float = 1.66
    hx_fm_ss316: float = 3.00             # Material factor SS316/SS316
    hx_fp_default: float = 1.15            # Pressure factor ~10 bar
    hx_u_salt_salt: float = 700.0          # Overall U salt-to-salt [W/m²K]
    hx_u_salt_zinc: float = 400.0          # Overall U salt-to-zinc (immersed coil) [W/m²K]
    hx_cost_multiplier: float = 1.0        # UA/cost sensitivity multiplier

    # ── Pump cost parameters ────────────────────────────────────────────
    # Source: Turton et al. (2018) + CSP molten salt premium (Turchi & Heath 2013)
    # Simplified power-law: C = pump_cost_a * W^pump_cost_b  [USD, kW]
    pump_cost_a: float = 10800.0           # Coefficient [USD]
    pump_cost_b: float = 0.62              # Exponent [-]
    pump_molten_salt_premium: float = 1.4  # Premium for Hastelloy parts, high-T seals
    pump_vfd_cost_per_kw: float = 250.0    # VFD cost [USD/kW]

    # ── EPC (Engineering, Procurement, Construction) overhead ───────────
    epc_overhead_fraction: float = 0.25    # 25% of total direct cost

    # ── Operational cost assumptions ───────────────────────────────────
    electricity_price_per_kwh: float = 0.17    # Chile industrial electricity [USD/kWh]
    aux_fuel_price_per_kwh: float = 0.06       # Chile industrial natural gas [USD/kWh_th]
    operating_hours_per_year: float = 3000.0   # Process operating hours [h/yr] (5d×12h×50wk)
